<div class="relative overflow-x-auto my-[1rem] min-h-[20rem]">
    ...
    <table class="text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 pt-[.5rem] display nowrap"
        id="dataTable2" width="100%" cellspacing="0" cellpadding="10">
        ...
        <tbody id="dataList">
            ...
            @foreach ($finance as $data)
                <tr>
                    ...
                    <td>
                        <button data-modal-target="editLabarugi{{ $data->id }}"
                            data-modal-toggle="editLabarugi{{ $data->id }}"
                            class="text-teal-400 border border-teal-400 p-1.5" type="button">
                            Edit
                        </button>

                        <div id="editLabarugi{{ $data->id }}" tabindex="-1" role="dialog"
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <div
                                        ...
                                        <button type="button"
                                            ...
                                            data-modal-hide="editLabarugi{{ $data->id }}">
                                    ...
                                    <div class="p-6 space-y-6">
                                        <form id="createForm" action="{{ route('labarugi_update', $data->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div class="flex justify-end mt-6">
                                                <button type="button"
                                                    ...
                                                    data-modal-hide="editLabarugi{{ $data->id }}">Close</button>
                                                <button type="submit"
                        ...
                        <button data-modal-target="labarugi{{ $data->id }}"
                            data-modal-toggle="labarugi{{ $data->id }}"
                            class="text-red-600 border border-red-600 p-1.5" type="button">
                            Hapus
                        </button>

                        <div id="labarugi{{ $data->id }}" tabindex="-1"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <button type="button"
                                        class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                        data-modal-hide="labarugi{{ $data->id }}">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                            Hapus?</h3>
                                        <form action="{{ url('labarugi_destroy/' . $data->id) }}" method="POST"
                                            style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button data-modal-hide="labarugi{{ $data->id }}" type="submit"
                                                class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                                Hapus
                                            </button>
                                        </form>
                                        <button data-modal-hide="labarugi{{ $data->id }}" type="button"
